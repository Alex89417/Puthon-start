...
# ● Модуль shutil
# Анонимные, lambda-функции
# На предыдущей лекции мы с Вами обговорили создание и использование функций в Python, но некоторые функции могут понадобиться всего 
# раз за всю работу приложения. Как можно обойтись без их явного описания? Как сократить подобный код?
# def f(x)
#   return x ** 2
# print(f(2))
# Функция в примере занимает всего две строчки кода, но в дальнейшем размеры описания функций будут увеличиваться. И
...
#   return x ** 2
# g = f
# Теперь в контексте этого приложения g может использоваться точно так же, как и f. g — это переменная, которая хранит в себе 
# ссылку на функцию.
# def f(x)
#   return x ** 2
# g = f
...
# Попробуем описать ту же логику для функции с двумя переменными. op — операция, воспринимаем её как отдельную функцию. 
# В примере это либо сумма (sum), либо перемножение(mylt):
# def sum(x, y):
#   return x + y
# def mylt(x, y):
#   return x * y
...
# Теперь, чтобы вызвать функцию суммы, достаточно просто вызвать sum.Также можно пропустить шаг создания переменной sum и сразу вызвать lambda:
